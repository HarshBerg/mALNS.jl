...
- Regret₅ Precise Insertion: `regret5precise!`
- Regret₅ Perturb Insertion: `regret5perturb!`
    ...
    for _ ∈ I
        ...
        for (j,v) ∈ enumerate(V)
            ...
            for _ ∈ 0:v.n
                ...
                z′ = round(Int, f(s) * (1 + φ * rand(rng, Uniform(-0.05, 0.05))))
                c  = z′ - z
...
The regret cost is calculated based on the difference between the best and other 
two insertion costs.
"""
...
"""
    regret5precise!(rng::AbstractRNG, s::Solution)

Returns solution `s` after inserting all open customer nodes into the solution `s`.
In each iteration, the algorithm selects the node with the maximum regret cost
and places it at its best position, evaluating insertion cost with precision. 
The regret cost is calculated based on the difference between the best and other 
five insertion costs.
"""
regret5precise!(rng::AbstractRNG, s::Solution) = regretk!(rng, s; k=5, mode=:precise)
"""
    regret5perturb!(rng::AbstractRNG, s::Solution)
...
five insertion costs.
"""
regret5perturb!(rng::AbstractRNG, s::Solution) = regretk!(rng, s; k=5, mode=:perturb)